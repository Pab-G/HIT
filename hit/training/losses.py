...
def compute_occupancy_loss(hit_pl, batch, pred_occ):
    ...
    Labels: 0=Femur, 1=Pelvis, 2=PeHumeruslvis, 3=Radius-Ulna, 4=Tibia-Fibula
    """
    num_classes = len(hit_pl.train_cfg.mri_labels)  # Should be 5
    ...
    labels = batch["mri_occ"].long() #- 3 #make 0-4

    # CrossEntropy for all classes (0-4)
    ...
    return occ_loss * hit_pl.train_cfg.lambda_occ
...
def compute_beta_disp_loss(hit_pl, batch, batch_idx, cond, step, train_cfg):
    ...
    v_c = hit_pl.smpl.forward_canonical(
        torch.zeros_like(betas)
    ).vertices  # Vertex on the canonical mesh
    ...
    beta_loss = F.mse_loss(
        v_c_pred * 100, v_c * 100
    )  # We pass the metric values in cm for better numerical stability
    ...
    if (
        hit_pl.current_epoch % hit_pl.cfg.export_canmeshes_every_n_epochs == 0
        and step == "train"
    ):
        mesh = hit_pl.smpl.psm(v_c_pred, display=False)
        mesh_path = mpimesh2glb(mesh)
        wandb.log({"train_can_space/unshape_pred": wandb.Object3D(mesh_path)})

        mesh = hit_pl.smpl.psm(v_c, v_c - v_c_pred, maxval=0.05, display=False)
        mesh_path = mpimesh2glb(mesh)
        wandb.log(
            {"train_can_space/beta_disp_with_pred_error": wandb.Object3D(mesh_path)}
        )
...
def Bdense_loss(x_s, x_c, B_xc, beta):
    fwd_disp = torch.matmul(
        B_xc.transpose(-2, -1),
        beta.unsqueeze(1).repeat(1, x_c.shape[1], 1).unsqueeze(-1),
    ).squeeze(-1)  # B, N, 3
...
def compute_linear_beta_loss(
    hit_pl, batch, cond, smpl_output, pts_c_inside, on_surface=False
):
    ...
    with torch.no_grad():
        if on_surface:
            ...
            x_c = hit_pl.smpl.forward_canonical(
                torch.zeros_like(batch["betas"])
            ).vertices

        else:
            x_m = batch["mri_points"]
            x_c = pts_c_inside
            x_s = hit_pl.hit.canonicalize_from_similar(
                x_m, x_c, smpl_output.tfs, cond, undo_shape=False
            )
    ...
    # import ipdb; ipdb.set_trace()
    B_flat = hit_pl.hit.fwd_beta(
        x_c, cond
    )  # note that fwd_beta is not conditioned on anything
    B_xc = B_flat.view(
        B_flat.shape[0], x_c.shape[1], 10, 3
    )  # B, N, 10, 3 The  predicted local B vector in x_c

    linear_beta_loss = Bdense_loss(
        x_s, x_c, B_xc, batch["betas"]
    )  # Forces the displacement to be a linear function of beta
...
def pose_bs_loss(hit_pl, batch, batch_idx, cond):
    ...
    smpl_posed = hit_pl.smpl.forward(
        zero_beta,
    ...
    xv_c_pbs = hit_pl.smpl.forward_shaped(
        zero_beta, batch["body_pose"], global_orient=None
    )
    ...
    if hit_pl.current_epoch % hit_pl.cfg.export_canmeshes_every_n_epochs == 0:
        ...
        mesh_path = mpimesh2glb(mesh)
        wandb.log(
            {"train_can_space/pose_dependant_blendshapes": wandb.Object3D(mesh_path)}
        )

        mesh = hit_pl.smpl.psm(xv_c_pbs - d, d, display=False)
        mesh_path = mpimesh2glb(mesh)
        wandb.log(
            {"gt_can_space/pose_dependant_blendshapes": wandb.Object3D(mesh_path)}
        )

    bs_loss = F.mse_loss(
        d_pred * 100, d * 100
    )  # Convert to cm with *100 for better numerical stability
...
def compute_outside_can_loss(hit_pl, smpl_output, batch, batch_idx):
    ...
    if "can_points" not in batch:
        raise DeprecationWarning(
            "The batch should contain can_points, the following is slow and deprecated"
        )
        ...
        pts_c_uniform = sample_uniform_from_max(
            (B, nb_points_canspace, 3), xf_p.max(), padding=0.30
        )
        if hit_pl.train_cfg["compressor"]:
            ...
            smpl_tfs = (
                smpl_output.tfs
            )  # get_gdna_bone_transfo(self.hit.smpl, smpl_output)
            xf_c = hit_pl.hit.canonicalize_from_similar(
                xf_p, smpl_template_verts, smpl_tfs, cond
            )
        ...
        def compute_outside_mask(x_c, faces, pts):
            ...
            for bi in range(B):
                mesh_can_f = trimesh.Trimesh(
                    vertices=x_c[bi].detach().cpu().numpy(), faces=faces, process=False
                )
                ...
                outside_mask[bi] = torch.tensor(
                    outside_bi, dtype=torch.bool, device=pts.device
                )
            return outside_mask
    ...
    uniform_can_pts_pred = hit_pl.forward(batch, pts_c_uniform, template=True)[
        "pred_occ"
    ]
    ...
    gt_outside_cc = torch.ones(nb_pts_outside).to(
        uniform_can_pts_pred.device
    )  # The class of all those points should be zero
    # import ipdb; ipdb.set_trace()
...
def compute_hands_loss(hit_pl, batch):
    ...
    loss_hands = F.cross_entropy(
        pred_hands.reshape(-1, len(hit_pl.train_cfg.mri_labels)),
        gt_hands_occ.long().reshape(-1),
    )
...
def compute_compression_loss(hit_pl, batch, batch_idx, smpl_output, cond, step="train"):
    ...
    xf_s = hit_pl.hit.canonicalize_from_similar(
        xf_p, smpl_template_verts, smpl_tfs, cond, undo_shape=False
    )
    # xf_c = hit_pl.hit.canonicalize_from_similar(xf_p, smpl_template_verts, smpl_tfs, cond, undo_shape=False)
    xv_s = hit_pl.smpl.forward_canonical(
        batch["betas"]
    ).vertices  # Vertex on the shaped mesh
    # compute shaped on

    d = xv_p - xf_p  # Ground truth decompression offset
    d_pred = hit_pl.hit.deformer.compressor(
        xf_s, cond
    )  # Predicted decompression offset
    ...
    if step == "train":
        ...
        hit_pl.log(
            "val_compression/d_err_max", torch.linalg.norm(d_pred - d, axis=-1).max()
        )
        ...
        if hit_pl.current_epoch % hit_pl.cfg.export_canmeshes_every_n_epochs == 0:
            ...
            mesh_path = mpimesh2glb(mesh)
            wandb.log(
                {"train_can_space/shape_space_compression": wandb.Object3D(mesh_path)}
            )

            mesh = hit_pl.smpl.psm(xf_p + d_pred, display=False)
            mesh_path = mpimesh2glb(mesh)
            wandb.log({"train_can_space/uncompressed_body": wandb.Object3D(mesh_path)})

            mesh = hit_pl.smpl.psm(xv_s, d, maxval=0.08, norm=False, display=False)
            mesh_path = mpimesh2glb(mesh)
            wandb.log(
                {"gt_can_space/shape_space_compression": wandb.Object3D(mesh_path)}
            )

            mesh = hit_pl.smpl.psm(
                xv_p, d - d_pred, maxval=0.08, norm=False, display=False
            )
            mesh_path = mpimesh2glb(mesh)
            wandb.log(
                {"gt_can_space/uncompressed_body_with_error": wandb.Object3D(mesh_path)}
            )
...
def compute_lbs_regularization_loss(hit_pl, batch, batch_idx, cond, step="train"):
    ...
    gt_weights = (
        hit_pl.smpl.lbs_weights.unsqueeze(0)
        .expand(batch["betas"].shape[0], -1, -1)
        .to(smpl_template_verts.device)
    )
    # import ipdb; ipdb.set_trace()
    # print(gt_weights.max())

    pred_weights = hit_pl.hit.deformer.query_weights(
        smpl_template_verts, {"latent": cond["lbs"]}
    )

    if (
        hit_pl.current_epoch % hit_pl.cfg.export_canmeshes_every_n_epochs
    ) == 0 and step == "train":
        mesh = hit_pl.smpl.psm(
            smpl_template_verts, skin_weights=pred_weights, display=False
        )
        mesh_path = mpimesh2glb(mesh)
        wandb.log({"train_can_space/can_lbs": wandb.Object3D(mesh_path)})

        mesh = hit_pl.smpl.psm(
            smpl_template_verts, skin_weights=gt_weights, display=False
        )
        mesh_path = mpimesh2glb(mesh)
        wandb.log({"gt_can_space/can_lbs": wandb.Object3D(mesh_path)})

        # difference
        mesh = hit_pl.smpl.psm(
            smpl_template_verts, pred_weights - gt_weights, display=False, norm=True
        )
        mesh_path = mpimesh2glb(mesh)
        wandb.log({"train_can_space/can_lbs_error": wandb.Object3D(mesh_path)})
    ...
    if hit_pl.train_cfg["lbs_loss_type"] == "mse":
        ...
        lbs_loss = lbs_loss + F.mse_loss(
            pred_weights[~non_zero_mask], gt_weights[~non_zero_mask]
        )
    else:
        raise ValueError(
            f"Unknown lbs loss type {hit_pl.train_cfg['lbs_loss_type']}, should be 'mse' or 'lbs_part_loss'"
        )
...
def compute_smooth_loss(hit_pl, batch, pts_c_inside):
    ...
    pred_occ_can = hit_pl.forward(batch, pts_c_inside, template=True, eval_mode=True)[
        "pred_occ"
    ]
...
def compute_surface_loss(hit_pl, batch, batch_idx, cond):
    ...
    smpl_can_verts = hit_pl.smpl.forward_canonical(
        torch.zeros_like(batch["betas"])
    ).vertices

    pred_occ = hit_pl.forward(batch, smpl_can_verts, template=True)["pred_occ"]
    pred_occ_NO = torch.softmax(pred_occ, dim=-1)[
        ..., 0
    ]  # softmax over the tissues, then take the first channel value (NO)
...
def compute_comp0_loss(hit_pl, batch, batch_idx, smpl_output, cond, pts_c, occ_pred):
    ...
    xf_s = hit_pl.hit.canonicalize_from_similar(
        pts_p, pts_c, smpl_tfs, cond, undo_shape=False
    )
    ...
    bone_comp_loss = F.mse_loss(
        d_pred[bone_mask], torch.zeros_like(d_pred[bone_mask])
    )  # Convert to cm with *100 for better numerical stability
    lt_comp_loss = F.mse_loss(
        d_pred[lt_mask], torch.zeros_like(d_pred[lt_mask])
    )  # Convert to cm with *100 for better numerical stability
    ...
    if hit_pl.train_cfg["comp0_loss_type"] == "bt_lt":
        ...
        hit_pl.train_cfg["comp0_loss_type"] == "norm"
    ):  # Just force the compression to be small
        comp0_loss = F.mse_loss(d_pred, torch.zeros_like(d_pred))
    else:
        raise ValueError(
            f"Unknown comp0 loss type {hit_pl.train_cfg['comp0_loss_type']}, should be 'bt_lt' or 'bt'"
        )
    ...
    if hit_pl.train_cfg["comp0_out"]:
        ...
        xf_s = hit_pl.hit.canonicalize_from_similar(
            mri_out_pts, pts_c, smpl_tfs, cond, undo_shape=False
        )
        d_pred = hit_pl.hit.deformer.compressor(xf_s, cond)
        # import ipdb; ipdb.set_trace()
        comp0_loss_out = F.mse_loss(
            d_pred[mri_out_pts] * 100, torch.zeros_like(d_pred[mri_out_pts])
        )  # Convert to cm with *100 for better numerical stability
...
def compute_mri_loss(hit_pl, batch, batch_idx, smpl_output, cond, pts_c_inside):
    ...
    nb_pts_outside = outside_can_mask.sum()
    gt_outside_cc = (
        torch.zeros(nb_pts_outside).to(uniform_mri_values_pred.device)
    ).type(torch.float32)
    ...
    loss_outside_mri = F.mse_loss(
        uniform_mri_values_pred[outside_can_mask], gt_outside_cc[..., None]
    )
    loss_outside_mri = (
        hit_pl.train_cfg.lambda_outmri * loss_outside_mri
    )  # weight the loss
    ...
    _, image = sl.plot_slice_value(
        mri_value, to_plot=False, mri_values=True
    )  # This is the SMPL frontal cut image of the mri value

    # log to wandb
    wandb.log({"train/mri_value_slice": wandb.Image(image)})
    # log mri_loss
    wandb.log({"train/mri_loss": mri_loss})
    # log outside loss
...
def compute_canonicalization_loss(
    hit_pl, batch, batch_idx, smpl_output, cond, step="train"
):
    ...
    xf_p = batch["body_verts_free"]
    xf_c = hit_pl.hit.deformer.query_cano(
        xf_p, {"betas": cond["betas"], "thetas": cond["thetas"]}
    )
    ...
    loss_canonicalization = (
        hit_pl.train_cfg.lambda_canonicalization * loss_canonicalization
    )
