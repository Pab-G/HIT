...
class SliceLevelSet:
    ...
    def plot_disp_field(
        ...
        if occupancy is not None:
            ...
            levels = matplotlib.ticker.MaxNLocator(nbins=self.nbins).tick_values(
                occ_val.min(), occ_val.max()
            )
        ...
        if quiver:
            ...
            qc = c[::qs, ::qs]
            plt.quiver(
                px, py, dx, dy, qc, cmap=cmap, angles="xy", scale_units="xy", scale=1
            )
        else:
            if twod_intensities:
            ...
            plt.streamplot(
                self.xx, self.yy, dx, dy, color=c, density=density, cmap=cmap
            )
    ...
    def plot_occupancy(self, occ):
        # 1. Update assertion to expect 5 Specialist Bone Classes
        # Original: assert len(occ.shape) == 3 and occ.shape[-1] == 4
        assert len(occ.shape) == 3 and occ.shape[-1] == 5

        # 2. Convert raw logits to class indices (0-4)
        occ = torch.sigmoid(occ)
        occ = torch.argmax(occ, dim=-1)

        # 3. Define a Specialist Bone Palette (RGB)
        # 0: Femur(Red), 1: Pelvis(Green), 2: Humerus(Blue), 3: Radius(Yellow), 4: Tibia(Magenta)
        specialist_palette = np.array(
            [
                [255, 0, 0],  # Red
                [0, 255, 0],  # Green
                [0, 0, 255],  # Blue
        ...
        # Color the labels using the new palette
        color_values = np.take(
        ...
        # Flip upside down to match MRI coordinates
    ...
    def plot_skinning_weights(self, sw, occupancy=None):
        ...
        plt.imshow(
            ...
            origin="lower",
            extent=(self.xbounds[0], self.xbounds[1], self.ybounds[0], self.ybounds[1]),
        )
    ...
    def plot_slice_value(self, values, to_plot=True, mri_values=False):
        ...
        if mri_values:
            ...
            if not type(values) == np.ndarray:
                values = (
                    values.reshape(self.xx.shape[0], self.xx.shape[1])
                    .detach()
                    .cpu()
                    .numpy()
                )
    ...
    def plot_slice_levelset(
        self, disp, values, to_plot=True, arrow_mode="quiver", iscompression=False
    ):
        """Given the values array (N,3), plot the level set"""
        ...
        ax2.streamplot(
            ...
            density=2,
            cmap=cmap,
        )  # , linewidth=lw Input is x,z,y so I should take the disp along x and y
...
if __name__ == "__main__":
    z0 = 0
    f = lambda pts: np.sin(pts[:, 0]) ** 10 + np.cos(
        10 + pts[:, 1] * pts[:, 1]
    ) * np.cos(pts[:, 0])
    g = lambda pts: pts
